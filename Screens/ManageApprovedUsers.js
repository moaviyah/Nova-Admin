...
const ManageApprovedUsers = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    onValue(usersRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const filteredUsers = userList.filter((user) => user.plan > 0);
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <Text style={{fontWeight:'bold', fontSize:20}}>Manage Approved Users</Text>
